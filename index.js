...
var Calendar = function (t) {
  ...
  Calendar.prototype.nextMonth = function () {
    ...
    for (var h, l = dm = "M" == this.f ? 1 : 0 == o ? -5 : 2, v = (c = 0, 0); v < 6; v++) {if (window.CP.shouldStopExecution(1)) break;
      ...
      for (var m = 0; m < 7; m++) {if (window.CP.shouldStopExecution(2)) break;
        ...
        {
          d += '<div class="cell' + (r == h && this.CurrentMonth == n && this.CurrentYear == a ? " active" : "") + '" onclick="myFunction(' + h + ',' + this.CurrentMonth + ',' + this.CurrentYear + ')"><span>' + h + "</span></div>", l = 1;
        }
        c % 7 == 6 && h >= i && (v = 10), c++;
  ...
  // onclick events for each date NOTE: January = 0, December = 11
  function myFunction(date, month, year) {
    let currentEvent = document.getElementById('' + date + '-' + month + '-' + year + '');
